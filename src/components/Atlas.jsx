...
const KeyboardShortcut = ({ keys, description, theme }) => (
  ...
  >
    ...
    <span
      style={{ color: theme === "dark" ? "#ccc" : "#666", marginLeft: "10px" }}
    >
      {description}
    </span>
  </div>
  ...
  const stats = useMemo(() => {
    ...
    const uniqueCountries = new Set(
      TRAVEL_DATA.map((loc) => {
        // 简单的国家判断（可以根据实际情况扩展）
        if (
          loc.city.en.includes("Bangkok") ||
          loc.city.en.includes("Koh Tao") ||
          loc.city.en.includes("Chiang Mai")
        )
          return "泰国";
        if (loc.city.en.includes("Seattle")) return "美国";
        if (loc.city.en.includes("Vancouver")) return "加拿大";
        if (
          loc.city.en.includes("Kauai") ||
          loc.city.en.includes("Honolulu") ||
          loc.city.en.includes("Big Island")
        )
          return "美国";
    ...
    for (let i = 0; i < TRAVEL_DATA.length - 1; i++) {
      ...
      const dLat =
        ((TRAVEL_DATA[i + 1].lat - TRAVEL_DATA[i].lat) * Math.PI) / 180;
      const dLng =
        ((TRAVEL_DATA[i + 1].lng - TRAVEL_DATA[i].lng) * Math.PI) / 180;
      const a =
        Math.sin(dLat / 2) * Math.sin(dLat / 2) +
        Math.cos((TRAVEL_DATA[i].lat * Math.PI) / 180) *
          Math.cos((TRAVEL_DATA[i + 1].lat * Math.PI) / 180) *
          Math.sin(dLng / 2) *
          Math.sin(dLng / 2);
    ...
    >
      ...
      <nav className="immersive-navbar">
        ...
        <div className="navbar-actions">
          ...
          {/* Expandable Search */}
          <div
            className={`minimalist-search-container ${
              isSearchExpanded
                ? "minimalist-search-expanded"
                : "minimalist-search-collapsed"
            }`}
          >
            <button
        ...
        <AnimatePresence>
          {showStats && (
            ...
            >
              ...
              >
                <div>
                  {lang === "zh" ? "首次旅行" : "First Trip"}: {stats.firstTrip}
                </div>
                <div>
                  {lang === "zh" ? "最近旅行" : "Latest Trip"}:{" "}
                  {stats.latestTrip}
                </div>
              </div>
            </motion.div>
                ...
                <div className="glass-filter-buttons">
                  ...
                  {getUniqueCountries().map((country) => (
                    <button
                      ...
                      className={`glass-filter-button ${
                        selectedCountry === country.en ? "active" : ""
                      }`}
                    >
      ...
      <AnimatePresence>
        {showKeyboardHelp && !isMobile && (
          ...
          >
            ...
            <div style={{ fontSize: "13px", lineHeight: "1.8" }}>
              ...
              <div>
                ...
                <KeyboardShortcut
                  keys={["?"]}
                  description={
                    lang === "zh" ? "显示/隐藏此面板" : "Show/hide this panel"
                  }
                  theme={theme}
                />
              </div>
            ...
            >
              {lang === "zh"
                ? "提示：支持 Vim 风格按键 (H J K L)"
                : "Tip: Vim-style keys supported (H J K L)"}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          {TRAVEL_DATA.map((item, index) => (
            <div
              ...
              title={
                lang === "zh"
                  ? "点击切换，双击查看日记"
                  : "Click to switch, Double-click to view diary"
              }
            >
                ...
                style={{
                  ...
                  background:
                    index === timelineIdx
                      ...
                      : "#3498db",
                  boxShadow:
                    index === timelineIdx ? "0 0 10px #ffeb3b" : "none",
                  transition: "all 0.3s",
                }}
              ></div>
            </div>
      ...
      >
        ...
        >
          <span style={{ fontSize: isMobile ? "14px" : "16px" }}>
            {TRAVEL_DATA[timelineIdx].country.code}
          </span>
        ...
        >
          ...
          <button
            onClick={() =>
              setTimelineIdx(Math.min(TRAVEL_DATA.length - 1, timelineIdx + 1))
            }
            ...
            style={{
              ...
              color: theme === "dark" ? "white" : "#2c3e50",
              cursor:
                timelineIdx === TRAVEL_DATA.length - 1
                  ? "not-allowed"
                  : "pointer",
        ...
        {selectedLoc && (
          ...
          >
            ...
            {selectedLoc.photos && selectedLoc.photos.length > 0 && (
              <div
                style={{
                  ...
                  display: "grid",
                  gridTemplateColumns:
                    isMobile || selectedLoc.photos.length === 1
                      ? "1fr"
                      : "repeat(2, 1fr)",
                  gap: "10px",
                }}
                ...
                {selectedLoc.photos.map((photo, idx) => (
                  <img
                    ...
                    style={{
                      width: "100%",
                      height:
                        selectedLoc.photos.length === 1 ? "250px" : "150px",
                      objectFit: "cover",
                      borderRadius: "6px",
                      cursor: "pointer",
                    }}
            ...
            <div
              style={{
                fontFamily:
                  lang === "zh" ? '"Songti SC", serif' : '"Georgia", serif',
                fontSize: "16px",
                lineHeight: "1.8",
        ...
        {showTip && (
          ...
          >
            ...
            >
              ...
              <span>
                {lang === "zh" ? "如何查看旅行日记？" : "How to view diary?"}
              </span>
            </div>
            ...
            >
              {lang === "zh" ? (
                <>
                  <div>
                    📖 <strong>点击底部城市名称</strong>
                  </div>
                  {!isMobile && (
                    <div>
                      📍 <strong>双击右侧时间轴</strong>
                    </div>
                  )}
                  <div>
                    🌍 <strong>点击地球上的标记点</strong>
                  </div>
                </>
              ) : (
                <>
                  <div>
                    📖 <strong>Click city name below</strong>
                  </div>
                  {!isMobile && (
                    <div>
                      📍 <strong>Double-click timeline</strong>
                    </div>
                  )}
                  <div>
                    🌍 <strong>Click marks on globe</strong>
                  </div>
                </>
              )}
            </div>
